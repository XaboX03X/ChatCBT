...
export default function AnxietyMeter({ currentScore }) {
  ...
  return (
    <div className="bg-white/60 backdrop-blur-xl w-full max-w-sm rounded-3xl shadow-xl border border-white p-8 flex flex-col items-center">
      ...
      <h3 className="text-sm font-bold text-slate-800 mb-6 uppercase tracking-wider">Anxiety Meter</h3>
      ...
      <div className="text-center mb-10">
        ...
        <span className="text-slate-400 text-xs font-semibold uppercase mt-2 block tracking-widest">Level Out of 10</span>
      </div>
      ...
      <div className="relative h-64 w-full flex items-center justify-center gap-8">
        ...
        <div className="w-10 h-64 rounded-full bg-slate-100 shadow-[inset_0_4px_10px_rgba(0,0,0,0.1)] relative overflow-hidden border border-slate-200/50">
          ...
          {/* THE RISING FLUID WRAPPER (This acts as the window revealing the color) */}
          <motion.div
            initial={{ height: "0%" }}
          ...
          >
            {/* THE LOCKED GRADIENT (Always fixed to h-64 so the colors don't squish) */}
        ...
        {/* Labels - Fixed Positioned Layout */}
        <div className="flex flex-col justify-between h-full py-2 text-xs font-bold text-slate-600">
            <div className="flex items-center h-[33%] text-[#f43f5e]">High Anxiety (7-10)</div>
            <div className="flex items-center h-[33%] text-[#fbbf24]">Moderate (4-6)</div>
            <div className="flex items-center h-[33%] text-[#2dd4bf]">Low (0-3)</div>
        </div>
      </div>
